...
func (command *commandAdopt) HandleSingle(resource *handler.Resource) error {
	...
	adoptOpts, err := ioutil.ReadFile(abs)
	if err != nil {
		return err
	}

	resource.Params.(*paramsAdopt).opts.Name = c.String("name")
	resource.Params.(*paramsAdopt).opts.AdoptStackData = string(adoptOpts)
	return nil
}
