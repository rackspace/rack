...
var get = cli.Command{
	...
	Description: "Retrieves an existing backup",
	Action:      actionGet,
...
func flagsGet() []cli.Flag {
	return []cli.Flag{
		cli.StringFlag{
			Name:  "id",
			Usage: "[optional; required if `stdin` isn't provided] The ID of the backup.",
		},
		cli.StringFlag{
			Name:  "stdin",
...
var keysGet = []string{"ID", "Name", "Description", "InstanceID", "Created",
	"Updated", "Size", "Status"}
...
func (command *commandGet) Execute(resource *handler.Resource) {
	...
	backup, err := backups.Get(command.Context().ServiceClient, id).Extract()
	if err != nil {
		resource.Err = err
		return
	}
