...
import (
	...
	"github.com/rackspace/rack/internal/github.com/rackspace/gophercloud/rackspace/db/v1/databases"
...
var remove = cli.Command{
	...
	Description: "Deletes an existing database from a given instance",
	Action:      actionDelete,
...
func (command *commandDelete) Execute(resource *handler.Resource) {
	...
	err := databases.Delete(command.Ctx.ServiceClient, params.instanceID, params.name).ExtractErr()
	if err != nil {
		resource.Err = err
		return
	}

	resource.Result = fmt.Sprintf("Database [%s] deleted from instance [%s]\n", params.name, params.instanceID)
}
