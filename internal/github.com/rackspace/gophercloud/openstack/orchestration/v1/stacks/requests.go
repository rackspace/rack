...
type CreateOpts struct {
	...
	// (OPTIONAL) A structure that contains details for the environement of the stack.
...
func (opts CreateOpts) ToStackCreateMap() (map[string]interface{}, error) {
	...
	if err := GetFileContents(opts.TemplateOpts, opts.TemplateOpts.ParsedTemplate, ignoreIfTemplate); err != nil {
			return nil, err
	}
	opts.TemplateOpts.FixFileRefs()
	s["template"] = string(opts.TemplateOpts.TemplateBin)
	...
	if opts.EnvironmentOpts != nil {
		...
		s["environment"] = string(opts.EnvironmentOpts.EnvironmentBin)
	}
